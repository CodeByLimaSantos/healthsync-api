...
public class FinancialService {
    ...
    public FinancialDTO createFinancial(CreateFinancialDTO dto) {
        // Validar customer/supplier de acordo com tipo
        ...
        Financial financial = financialMapper.convertToEntity(dto);
        ...
        return financialMapper.convertToBasicDTO(savedFinancial);
    }
    ...
    public FinancialDTO findById(Long id) {
        ...
        return financialMapper.convertToBasicDTO(financial);
    }
    ...
    public List<FinancialDTO> findAll() {
        ...
        return financialMapper.convertToBasicDTOList(financials);
    }
    
    /**
     * Lista lançamentos pendentes (não pagos)
     */
    @Transactional(readOnly = true)
    ...
    public FinancialDTO update(Long id, CreateFinancialDTO dto) {
        ...
        return financialMapper.convertToBasicDTO(updatedFinancial);
    }
    ...
    /**
     * Deleta um lançamento (apenas os não pagos)
     */
    ...
    /**
     * Gera resumo financeiro do sistema
     */
    @Transactional(readOnly = true)
    public FinancialSummaryDTO generateSummary() {
        ...
        summary.setCountPendingReceivable(receivable.stream().filter(f -> f.getStatus() == PaymentStatus.PENDING).toList().size());
        summary.setCountPendingPayable(payable.stream().filter(f -> f.getStatus() == PaymentStatus.PENDING).toList().size());
        summary.setCountOverdueReceivable(receivable.stream().filter(Financial::isOverdue).toList().size());
        summary.setCountOverduePayable(payable.stream().filter(Financial::isOverdue).toList().size());
